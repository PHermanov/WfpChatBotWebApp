    ...
    : IOpenAiChatService
{
        ...
        if (toolCalls.Count != 0)
        {
            ...
            foreach (var toolCall in toolCalls)
            {
                ...
                await foreach (var toolOutput in openAiChatToolsService.GetToolCallOutput(toolCall, cancellationToken))
                {
                    ...
                    if(toolCall.FunctionName == nameof(IAiImageService.CreateImage))
                        yield return toolOutput;
                }

                if (toolCall.FunctionName == "web_search")
                {
                    var finalResponse = openAiClientFactory.ChatClient.CompleteChatStreamingAsync(
                        messages: messagesQueue.ToArray(),
                        options: completionOptions,
                        cancellationToken: cancellationToken);

                    StringBuilder contentBuilderFinal = new();

                    await foreach (var completion in finalResponse)
                    {
                        foreach (var contentPart in completion.ContentUpdate)
                        {
                            contentBuilderFinal.Append(contentPart.Text);

                            yield return new OpenAiResponse
                            {
                                ContentType = OpenAiContentType.Text,
                                Content = contentBuilderFinal.ToString(),
                                ContentComplete = false
                            };
                        }
                    }

                    if (contentBuilderFinal.Length != 0)
                    {
                        yield return new OpenAiResponse
                        {
                            ContentType = OpenAiContentType.Text,
    ...
    private async ValueTask<ChatMessage> CreateChatMessage(OpenAiRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var mediaType = request.Image.GetImageMediaType();
