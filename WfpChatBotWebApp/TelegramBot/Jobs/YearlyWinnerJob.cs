    ...
    : IRequestHandler<YearlyWinnerJobRequest>
{
        ...
        try
        {
            ...
            if (yearWinnerByCount != null && !string.IsNullOrEmpty(yearWinnerByCountMessageTemplate))
            {
                ...
                if (user != null)
                {
                    ...
                    await botClient.TrySendTextMessageAsync(chatId, yearWinnerByCountMessage, cancellationToken: cancellationToken);
                }
                else
                {
            ...
            if (monthWinners.Count != 0)
            {
                ...
                if (yearWinner != null && yearWinnerUser != null)
                {
                    ...
                    {
                        await Task.Delay(500, cancellationToken);
                        await botClient.TrySendTextMessageAsync(chatId, yearAllMonthWinnersMsg, cancellationToken: cancellationToken);
                        await Task.Delay(500, cancellationToken);
                        await botClient.TrySendTextMessageAsync(chatId, monthStrRes.ToString(), cancellationToken: cancellationToken);
                    }
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrEmpty(mainYearWinnerMultiple))
                        {
                            ...
                            await botClient.TrySendTextMessageAsync(chatId, msg, cancellationToken: cancellationToken);
                        }
    ...
    private async Task SendPicture(long chatId, PlayerCountViewModel player, string msg, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            userProfilePhotos = await botClient.GetUserProfilePhotosAsync(player.UserId, cancellationToken: cancellationToken);
        }
        catch (Exception e)
        {
